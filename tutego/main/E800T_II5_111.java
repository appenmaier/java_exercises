...
public class E800T_II5_111 {
   ...
   public static void main(String[] args) {
      ...
      // 3
      System.out.println("Terminal 3: " + heroes.stream()
            .anyMatch(t -> t.yearFirstAppearance >= 1960 && t.sex == Hero.Sex.FEMALE));
      ...
      heroes.stream()
            .filter(t -> t.sex == Sex.FEMALE ? true : false)
            .map(t -> t.name)
      ...
      List<Hero> heroesAnonymized = heroes.stream()
            .map(t -> new Hero(t.name.replaceAll("\\s*\\(.*\\)$", ""),
                  t.sex,
                  t.yearFirstAppearance))
