...
public class DiceGame {
   ...
   private void move(Player player) {
      ...
      if (player.getSpecialAvailable()) {
         System.out.print(player.getName()
               + ", moechtest Du einmalig Spezialwuerfel verwenden (1=ja, 2=nein)?: ");
   ...
   /*
    * version 1.0: private void move(Player player) { dices.add(new Dice(1)); dices.add(new
    * Dice(2)); dices.add(new Dice(3)); System.out.print(player.getName() +
    * ", moechtest Du wuerfeln (true, false)?: "); if (scanner.nextBoolean()) {
    * diceCup.rollTheDices(dices); int score = 0; for (Dice dice : dices) { score +=
    * dice.getValue(); } System.out.println(player.getName() + " hat " + score + " Punkte");
